...
files <- list.files(path="./16-sep/ref/", pattern="*.xlsx", full.names=T, recursive=FALSE)

ref_data_set <- do.call(rbind,lapply(files, readrefdata ))
names(ref_data_set) <- c("Panel", "College" , "Location" , "Name" ,   "Email" , "ProgrammingAtCollege" , "TechMCQ" , "Aptitude")
refdata_to_merge <- ref_data_set %>% select (Email , ProgrammingAtCollege , TechMCQ , Aptitude) 

report_df <- merge(master_data_set, refdata_to_merge, by="Email", all.x=TRUE) 

report_df <- report_df %>% mutate(
    aggregated_score = TeamPlayer * (10/100) + 
      GroupCommunication * (10/100) + 
      ProblemSolving * (40/100) + 
      F2FCommunication * (20/100) + 
      Programming * (3/100) * (20/100)
)
